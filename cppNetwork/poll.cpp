...
int func(int argc, char* argv[])
{
    ...
    int sockfd = socket(AF_INET, SOCK_STREAM, 0);
    if(sockfd < 0)      //sockfd 这里的取值规则
    {
        errlog<<"create err." <<std::endl;
        return -1;
    }
    ...
    while (1)
    {
        ...
        {
            int clientfd = accept(sockfd, (struct sockaddr*)&clientAddr, &clientLen);
            if(clientfd < 0)    //非阻塞IO到这里也不会失败,判断了fds[sockfd].revents & POLLIN
            {
                errlog<<"accept err. sockfd:"<<sockfd<<std::endl;
                continue;
            }
